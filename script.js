...
document.getElementById("add-recipe-btn").addEventListener("click", async () => {
  ...
  const recipeImage = document.getElementById("recipeImage").value.trim(); // <-- New field
  const recipeInstructions = document.getElementById("recipeInstructions")?.value.trim(); // Optional, if present
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (document.getElementById("recipeInstructions")) {
